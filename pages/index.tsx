...
export default function AssistantStudio() {
  ...
  const sendMessage = async (promptOverride?: string) => {
    ...
    const updatedTitle = isFirstUserMessage ? textToSend.slice(0, 32) + '...' : activeSession.title;
  ...
  return (
    <div className="h-screen bg-slate-50 text-slate-900 font-sans flex overflow-hidden selection:bg-cyan-100 selection:text-cyan-950">
      <Head>
      ...
      {/* LEFT SIDEBAR */}
      <aside
        className={`w-72 bg-white border-r border-slate-200 flex flex-col justify-between transition-all duration-300 z-30 shrink-0 ${
      ...
      >
        <div className="p-4 space-y-4 overflow-y-auto">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="p-1 rounded-lg hover:bg-slate-100 text-slate-400 md:hidden"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <button
            ...
            className="w-full py-2.5 px-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition"
          >
            <Plus className="h-4 w-4" />
            {sidebarOpen && <span>New Advisory Chat</span>}
          </button>
                ...
                {sessions.map((s) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 hover:text-red-600 transition p-1"
                    >
        ...
        {sidebarOpen && (
          <div className="p-3 border-t border-slate-200 bg-slate-50/50 space-y-1.5 text-[11px]">
            ...
            <div className="space-y-1">
              <a href="https://toolsverse-kappa.vercel.app/" target="_blank" className="flex items-center justify-between p-1.5 rounded-lg hover:bg-white text-slate-700 font-semibold transition border border-transparent hover:border-slate-200">
                <span>ToolVerse</span> <ExternalLink className="h-3 w-3 text-slate-400" />
              </a>
              <a href="https://myimagetools.netlify.app/" target="_blank" className="flex items-center justify-between p-1.5 rounded-lg hover:bg-white text-slate-700 font-semibold transition border border-transparent hover:border-slate-200">
                <span>MyImageTools</span> <ExternalLink className="h-3 w-3 text-slate-400" />
              </a>
              <a href="https://agency-hub-pink.vercel.app/" target="_blank" className="flex items-center justify-between p-1.5 rounded-lg hover:bg-white text-slate-700 font-semibold transition border border-transparent hover:border-slate-200">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="h-16 border-b border-slate-200 bg-white/95 px-5 flex items-center justify-between shrink-0 shadow-2xs">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2 rounded-xl border border-slate-200 hover:bg-slate-100 text-slate-600 md:hidden"
            >
              <Menu className="h-5 w-5" />
            </button>
            <div>
              <h2 className="text-sm font-extrabold text-slate-900">{activeSession?.title}</h2>
              <p className="text-[11px] text-slate-500 font-medium">Session Memory Synchronized · Rajshahi, BD</p>
            </div>
          </div>
        ...
        {/* Scrollable Conversation Stream with Clean Markdown Rendering */}
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 max-w-4xl w-full mx-auto">
          {activeSession?.messages.length <= 2 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-bold">
                ...
                >
                  <span>📝 Draft 3-Sentence DM for Lead #1</span>
                ...
                <button
                  ...
                  className="text-left p-3 rounded-xl border border-slate-200 hover:border-cyan-500 hover:bg-cyan-50/40 transition flex items-center justify-between text-slate-800"
                >
                  <span>🔥 Review My Top Ranked Leads</span>
                  <ChevronRight className="h-4 w-4 text-slate-400" />
                </button>
              </div>
            </div>
          ...
          {activeSession?.messages.map((m, idx) => (
            ...
            >
              ...
              <div
                className={`p-5 rounded-2xl max-w-2xl text-sm leading-relaxed relative group ${
                  m.role === 'user'
                    ? 'bg-cyan-600 text-white rounded-br-xs shadow-xs font-medium'
                    : 'bg-white border border-slate-200 text-slate-800 rounded-bl-xs shadow-xs'
                }`}
              >
                {m.image && (
                  <img src={m.image} alt="Uploaded preview" className="rounded-xl mb-3 max-h-72 object-contain border border-slate-200" />
                )}

                {/* Markdown Renderer: Removes raw asterisks & renders clean HTML typography */}
                <div className="prose prose-sm max-w-none text-slate-800 leading-relaxed space-y-2">
                  <ReactMarkdown
                    components={{
                ...
                {m.role === 'assistant' && (
                  <button
                    ...
                    className="absolute top-3 right-3 p-1.5 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-500 opacity-0 group-hover:opacity-100 transition"
                    title="Copy Answer"
                  >
          ...
          {loading && (
            <div className="flex items-center gap-2.5 text-xs font-bold text-cyan-800 bg-cyan-50/80 p-3.5 rounded-2xl border border-cyan-200 w-fit shadow-xs">
              <RefreshCw className="h-4 w-4 animate-spin text-cyan-600" />
              <span>Analyzing live database context and drafting proposal...</span>
            </div>
          )}
        ...
        <footer className="border-t border-slate-200 bg-white p-4 shrink-0">
          <div className="max-w-4xl mx-auto space-y-2">
            {selectedImage && (
              <div className="flex items-center gap-2 p-2 bg-slate-100 rounded-xl border border-slate-200 w-fit">
                <img src={selectedImage} alt="Attachment" className="h-10 w-10 object-cover rounded-lg" />
                <span className="text-xs text-slate-700 font-semibold">Screenshot attached</span>
                <button onClick={() => setSelectedImage(null)} className="text-xs text-red-600 font-bold ml-2 hover:underline">Remove</button>
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-3 rounded-xl border border-slate-200 hover:bg-slate-100 text-slate-600 transition"
              >
              ...
              <input
                type="text"
                placeholder="Ask for a 3-sentence proposal, client advice, or task review..."
                value={input}
                onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && sendMessage()}
                className="flex-1 bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-hidden focus:border-cyan-600 focus:bg-white transition font-medium"
              />

              <button
                onClick={() => sendMessage()}
                disabled={loading || (!input.trim() && !selectedImage)}
                className="px-5 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-sm font-bold flex items-center gap-2 transition disabled:opacity-40 shadow-xs"
              >
                <span>Send</span>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-extrabold text-slate-900">Personal Memory Vault</h3>
              </div>
              <button onClick={() => setShowMemoryModal(false)} className="text-slate-400 hover:text-slate-600">
                <X className="h-5 w-5" />
              </button>
            </div>

            <p className="text-xs text-slate-500">
              Auto-synced with live Supabase counts. You can add notes or rules below to inject them into the AI on every prompt.
            </p>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-bold border border-slate-200 hover:bg-slate-100 text-slate-700"
                >
                  Close
                </button>
                <button
                  onClick={saveMemoryVault}
                  className="px-4 py-2 rounded-xl text-xs font-bold bg-cyan-600 hover:bg-cyan-500 text-white shadow-xs"
                >
                  Save Changes
                </button>
              </div>
